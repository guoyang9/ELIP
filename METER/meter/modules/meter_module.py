...
from .clip_model_vl_pruning import build_model, adapt_position_encoding
...
class METERTransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        if self.is_clip:
            ...
            patch_size = 32
            num_tokens = (config['resolution_before'] // patch_size) ** 2
            prune_state = dict(
            ...
            self.vit_model = build_model(config['vit'], resolution_after=resolution_after, prune_state=prune_state) # switch
    ...
    def infer(
        ...
        image_embeds = self.vit_model(img, text_cls=text_cls) 
