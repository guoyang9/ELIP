...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, normalize: bool = False, text_cls=None):
        features = self.visual(image, text_cls=text_cls)
        return F.normalize(features, dim=-1) if normalize else features
    
    def encode_text(self, text, normalize: bool = False, output_raw=False):
        ...
        if output_raw:
            if self.text_projection is not None:
                if isinstance(self.text_projection, nn.Linear):
                    projected_x = self.text_projection(x)
                else:
                    projected_x = x @ self.text_projection

            return (F.normalize(projected_x, dim=-1), raw_x) if normalize else (projected_x, raw_x)
        else:
            if self.text_projection is not None:
                if isinstance(self.text_projection, nn.Linear):
    ...
    def forward(
        ...
        image_features = self.encode_image(image, normalize=True, text_cls=text_features.clone().detach()) if image is not None else None
