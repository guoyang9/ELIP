...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        if output_attn:
            out, attn_weights = self.attn(
                q_x, k_x, v_x, need_weights=output_attn, attn_mask=attn_mask
            )
    ...
    def forward(
        ...
        if output_attn:
            out, attn_weights = self.attention(q_x=self.ln_1(q_x), k_x=k_x, v_x=v_x, attn_mask=attn_mask, output_attn=output_attn)
            x = q_x + self.ls_1(out)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        num_tokens = 197
        self.prune_state = dict(
            begin_layer=[2, 4, 10],
            num_act_layers=[2, 6, 2],
            num_keep_tokens=[int(num_tokens * prob) for prob in [0.90, 0.65, 0.40]]
        )
    ...
    def forward(self, x: torch.Tensor, text_cls=None):
        x = self._embeds(x)

        if text_cls is not None:
            with torch.no_grad():
                text_cls = text_cls @ self.proj.T

        x = self.vision_token_pruning(x, text_cls=text_cls) # start token pruning
    ...
    def vision_token_pruning(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, text_cls=None):
        if not self.transformer.batch_first:
            x = x.transpose(0, 1).contiguous()    # NLD -> LND
        
        # implementation of ELIP starts here
            ...
            if text_cls is not None and begin_layer is not None and i in begin_layer:
                ...
                with torch.no_grad():
                    ...
                    for layer in range(num_layers):
                        output_attn = True if layer == (num_layers - 1) else False
                        block = copy.deepcopy(self.transformer.resblocks[i + layer])

                        x_temp = block(x_temp, output_attn=output_attn)

                    attn_blk = x_temp[1][:, 0, 1:] # heads already averaged, skipping cls token
